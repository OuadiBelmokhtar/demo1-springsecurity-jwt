...
public class UserAccountRestController {
    ...
    public void refreshToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
        System.out.println("UserAccountRestController.refreshToken");
        ...
        if (refreshToken != null && refreshToken.startsWith("Bearer ")) {
            try {
                ...
                String renewedJwtAccessToken = JWT.create().withSubject(username)
                        ...
                        .withClaim("roles", appUser.getUserRoles().stream().map(role -> role.getRoleName())
    ...
    /*
      - Permet de récupérer le user authentifié de la BD sachant le username.
      - Le param 'principal' sera injecté par Spring
      - Ca necessite une authentification bien evidemment
     */
    @GetMapping(path = "/userProfile")
    AppUser authenticatedUserProfile(Principal principal) {
        return userAccountService.loadUserByUserName(principal.getName());
    }
}
